      ...
      <el-tab-pane :label="$t('trade.openOrder')" name="first">
        ...
        >
          ...
          <el-table-column prop="ordervol" :label="$t('trade.orderVol')" width="150">
            <template slot-scope="scope">
              <span>{{ scope.row.orderVol }} {{ scope.row.pair}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="ordertotal" :label="$t('trade.orderTotal')" width="150">
            <template slot-scope="scope">
              <span>{{ handFloat(NH.times(scope.row.orderVol, scope.row.orderPrice), 4) }} {{ scope.row.code === '3' ? 'BDE' : (scope.row.code === '2' ? ' ETH' : ' EOS') }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="dealtvol" :label="$t('trade.dealtVol')" width="150">
            <template slot-scope="scope">
              <span>{{ scope.row.dealVol }} {{ scope.row.pair }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="pendingVol" :label="$t('trade.pendingVol')" width="150">
            <template slot-scope="scope">
              <span>{{ scope.row.pendingVol }} {{ scope.row.pair }}</span>
            </template>
          </el-table-column>
